...
http.DefaultRequestHeaders.UserAgent.ParseAdd("MyOnline-TV-Web/0.5.0");
...
app.MapGet("/ready", () =>
{
    ...
    return ready
        ? Results.Ok(new { status = "ready", version = "0.5.0", checks })
        : Results.Json(new { status = "not-ready", version = "0.5.0", checks }, statusCode: 503);
}).AllowAnonymous();
...
async Task<JsonDocument> XtreamJson(ProviderConnection c, string action, TimeSpan timeout, (string Key, string Value)? extra = null)
{
    ...
    request.Headers.TryAddWithoutValidation("User-Agent", "MyOnline-TV/0.5.0");

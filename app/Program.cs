...
http.DefaultRequestHeaders.UserAgent.ParseAdd("MyOnline-TV-Web/0.3.4");
...
app.MapGet("/ready", () =>
{
    ...
    return ready
        ? Results.Ok(new { status = "ready", version = "0.3.4", checks })
        : Results.Json(new { status = "not-ready", version = "0.3.4", checks }, statusCode: 503);
}).AllowAnonymous();

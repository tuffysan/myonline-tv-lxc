...
http.DefaultRequestHeaders.UserAgent.ParseAdd("MyOnline-TV-Web/6.2.0");
...
app.MapGet("/ready", () =>
{
    ...
    return ready
        ? Results.Ok(new { status = "ready", version = "6.2.0", checks })
        : Results.Json(new { status = "not-ready", version = "6.2.0", checks }, statusCode: 503);
}).AllowAnonymous();
...
app.MapGet("/api/appliance/health", () =>
{
    ...
    return Results.Ok(new {
        version="6.2.0", dataDirectory=dataDir,
...
app.MapGet("/api/platform/status", () =>
{
    var drive=new DriveInfo(Path.GetPathRoot(dataDir)!);
    return Results.Ok(new {
        version="6.2.0",platform="MyOnline TV Platform",
        providers=LoadProviders().Count,
...
app.MapGet("/api/system/migration-manifest",(HttpContext ctx)=>{
 ...
 return Results.Ok(new{version="6.2.0",created=DateTimeOffset.UtcNow,dataDirectory=dataDir,configurationFiles=files,
...
app.MapGet("/api/appliance/readiness",async ()=>{
 ...
 await Task.CompletedTask;
 return Results.Ok(new{version="6.2.0",ready=ffmpeg&&ffprobe&&dataWritable&&LoadUsers().Count>0,checks});
}).RequireAuthorization(p=>p.RequireRole("Admin"));
app.MapGet("/api/appliance/version",()=>Results.Ok(new{product="MyOnline TV",version="6.2.0",channel="stable",platform="LXC"}));
...
async Task<JsonDocument> XtreamJson(ProviderConnection c, string action, TimeSpan timeout, (string Key, string Value)? extra = null)
{
    ...
    request.Headers.TryAddWithoutValidation("User-Agent", "MyOnline-TV/6.2.0");

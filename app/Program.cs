...
builder.WebHost.UseUrls("http://128.0.1.1:5080");
...
http.DefaultRequestHeaders.UserAgent.ParseAdd("MyOnline-TV-Web/8.0.1");
...
app.MapGet("/ready", () =>
{
    ...
    return ready
        ? Results.Ok(new { status = "ready", version = "8.0.1", checks })
        : Results.Json(new { status = "not-ready", version = "8.0.1", checks }, statusCode: 503);
}).AllowAnonymous();
...
app.MapGet("/api/appliance/health", () =>
{
    ...
    return Results.Ok(new {
        version="8.0.1", dataDirectory=dataDir,
...
NavigationConfig LoadNavigationConfig()
{
    ...
    var ids=new[]{"home","live","guide","movies","series","plex","jellyfin","downloads","recordings","notifications","rooms","library","search","system","platform","diagnostics","appliance","admin"};
...
app.MapPut("/api/admin/navigation",(NavigationConfig input)=>{
    var allowed=new HashSet<string>(new[]{"home","live","guide","movies","series","plex","jellyfin","downloads","recordings","notifications","rooms","library","search","system","platform","diagnostics","appliance","admin"},StringComparer.OrdinalIgnoreCase);
...
app.MapGet("/api/platform/status", () =>
{
    var drive=new DriveInfo(Path.GetPathRoot(dataDir)!);
    return Results.Ok(new {
        version="8.0.1",platform="MyOnline TV Platform",
        providers=LoadProviders().Count,
...
// v8.0.1 Source & Access Architecture
...
// v8.0.1 Backup, Restore & Migration
app.MapGet("/api/system/migration-manifest",(HttpContext ctx)=>{
 ...
 return Results.Ok(new{version="8.0.1",created=DateTimeOffset.UtcNow,dataDirectory=dataDir,configurationFiles=files,
...
app.MapGet("/api/appliance/readiness",async ()=>{
 ...
 await Task.CompletedTask;
 return Results.Ok(new{version="8.0.1",ready=ffmpeg&&ffprobe&&dataWritable&&LoadUsers().Count>0,checks});
}).RequireAuthorization(p=>p.RequireRole("Admin"));
app.MapGet("/api/appliance/version",()=>Results.Ok(new{product="MyOnline TV",version="8.0.1",channel="stable",platform="LXC"}));
...
async Task<JsonDocument> XtreamJson(ProviderConnection c, string action, TimeSpan timeout, (string Key, string Value)? extra = null)
{
    ...
    request.Headers.TryAddWithoutValidation("User-Agent", "MyOnline-TV/8.0.1");

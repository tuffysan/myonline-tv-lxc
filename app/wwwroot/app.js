...
function installVodRecovery(video,getHls,savePosition){
  ...
  const arm=(allowPaused=false)=>{
    ...
    // v41.0.4: browser-generated pauses during an underrun must remain recoverable.
    // Previously pause -> arm() immediately returned because video.paused was true,
    // leaving VOD permanently stopped until the user pressed Play.
    stallTimer=setTimeout(async()=>{
      stallTimer=null;if(video.ended||video.dataset.userPaused==='1')return;
      const now=bufferedEnd();
      if(now>lastBufferedEnd+.5){
        if(video.paused)await video.play().catch(()=>{});
        status(video.paused?'Buffering…':'Playing');
        if(video.paused)arm(true);
        return;
      }
      const instance=getHls?.();recoveryStage++;
      try{
        if(instance&&recoveryStage===1){status('Buffering · reconnecting…');instance.startLoad(-1);return arm(true)}
        if(instance&&recoveryStage===2){status('Buffering · recovering media…');instance.recoverMediaError();return arm(true)}
        status('Stream stalled · retrying playback…');
  ...
  // v41.0.2: pause events can be generated by the browser while the HLS buffer
  // is being replenished. Only the player UI marks an intentional user pause.
...
async function playServerMedia(token,name,mediaId=null,forceTranscode=false,poster='',startAtSeconds=0){
  ...
  try{
    ...
    if(window.Hls&&Hls.isSupported()){
      hls=new Hls({enableWorker:true,lowLatencyMode:false,backBufferLength:120,maxBufferLength:VOD_BUFFER_TARGET_SECONDS,maxMaxBufferLength:240,maxBufferSize:120*1000*1000,maxBufferHole:0.5,startFragPrefetch:true,fragLoadingMaxRetry:8,manifestLoadingMaxRetry:6,levelLoadingMaxRetry:6});
      hls.loadSource(playbackUrl);hls.attachMedia(video);

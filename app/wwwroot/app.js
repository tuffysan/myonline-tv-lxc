...
function lmApplyGroupFilters(){const q=($('#lmSearch')?.value||'').toLowerCase(),s=$('#lmGroupState')?.value||'all';document.querySelectorAll('.lmGroupCard[data-lm-filter]').forEach(x=>{const text=x.dataset.lmFilter||'',hidden=x.dataset.hidden==='true';x.hidden=!(text.includes(q)&&(s==='all'||(s==='active'&&!hidden)||(s==='inactive'&&hidden)))})}
async function lmSetVisibleGroups(hidden){const names=[...document.querySelectorAll('.lmGroupCard[data-lm-filter]:not([hidden])')].map(x=>x.querySelector('.lmTitle b')?.textContent||'').filter(Boolean);if(!names.length)return;if(!confirm(`${hidden?'Deactivate':'Activate'} ${names.length} matching groups?`))return;for(const group of names)await jpost('/api/channel-preferences/'+encodeURIComponent(lmProviderId())+'/group',{group,hidden});await libraryManagementView()}

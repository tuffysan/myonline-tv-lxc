...
function installVodBufferMonitor(video){
  ...
  const update=()=>{
    ...
    const ahead=bufferedAheadSeconds(video);
    if(status&&!video.ended&&video.dataset.userPaused!=='1'&&ahead<VOD_BUFFER_FLOOR_SECONDS){
      status.textContent=ahead<3?'Buffering · replenishing…':'Playing · building buffer…';
    }
  };
...
function installVodRecovery(video,getHls,savePosition){
  ...
  video.addEventListener('pause',()=>{
    if(video.dataset.userPaused!=='1'&&!video.ended){status('Buffering…');arm()}
  });
...
async function playServerMedia(token,name,mediaId=null,forceTranscode=false,poster='',startAtSeconds=0){
  ...
  try{
    ...
    if(seekBar&&mediaDurationSeconds>0){
      ...
      const requestInstantSeek=(target)=>{
        ...
        seekTimer=setTimeout(async()=>{
          ...
          try{
            ...
            if(hls){hls.stopLoad();hls.loadSource(nextUrl);hls.startLoad(-1)}else{video.src=nextUrl;video.load()}
            await video.play().catch(()=>{});
      ...
      seekBar.addEventListener('input',()=>{const target=Number(seekBar.value)||0,c=$('#mediaCurrentTime');if(c)c.textContent=formatMediaTime(target);requestInstantSeek(target)});
